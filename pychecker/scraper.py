...
def update(url):
	...
	limits = { "global": {"daily":0, "hourly":0}, "domain":{"daily":0,"hourly":0}, "site":{"daily":0,"hourly":0}}
        for row in r:
	...
	if p is not None:
		limits["site"]["daily"] = db_session.query(ScrapeHistory).filter(ScrapeHistory.product_id == p.id and ScrapeHistory.created >= 60*60*24).count() 
	        limits["site"]["hourly"] = db_session.query(ScrapeHistory).filter(ScrapeHistory.product_id == p.id and ScrapeHistory.created >= 60*60).count()
	else:
		limits["site"]["daily"] = 0
	...
	if limits["domain"]["daily"] >= getRateLimit("domain", "daily"): raise Exception("Daily rate limit for domain reached")
	if limits["domain"]["hourly"] >= getRateLimit("domain", "hourly"): raise Exception("Hourly rate limit for domain reached")
	if limits["site"]["daily"] >= getRateLimit("site", "daily"): raise Exception("Daily rate limit for url reached")
	if limits["site"]["hourly"] >= getRateLimit("site", "hourly"): raise Exception("Hourly rate limit for url reached")
	if limits["global"]["daily"] >= getRateLimit("global", "daily"): raise Exception("Daily global rate limit reached")
	if limits["global"]["hourly"] >= getRateLimit("global", "hourly"): raise Exception("Hourly global rate limit reached")
	...
	for row in r:
		...
		if p is not None: s = ScrapeHistory(p.id, row.id, "Success", price)
		else: s = ScrapeHistory(0, row.id, "Success", price)
		db_session.add(s)
...
def process():
	...
	if os.path.exists( lockfile ): raise Exception("Scraper already running")
